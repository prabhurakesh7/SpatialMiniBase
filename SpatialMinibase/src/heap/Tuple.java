...
public class Tuple implements GlobalConst
{
	 ...
	 */
	public double[] getDoubleArrayFld(int fldNo) throws IOException,
	        FieldNumberOutOfBoundException
	{
		double[] val;
		if ((fldNo > 0) && (fldNo <= fldCnt))
		{
			val = Convert.getDoubleArrValue(fldOffset[fldNo - 1], data,
	 ...
	 * @throws FieldNumberOutOfBoundException
	 */
	public Tuple setDoubleArrayFld(int fldNo, double[] val) throws IOException,
	        FieldNumberOutOfBoundException
	{
		if ((fldNo > 0) && (fldNo <= fldCnt))
		{
			Convert.setDoubleArrValue(val, fldOffset[fldNo - 1], data);
			return this;
		}
		else
			throw new FieldNumberOutOfBoundException(null,
	 ...
	 * @throws FieldNumberOutOfBoundException
	 */
	public Tuple setSpatialGeomtryFld(int fldNo, SpatialGeometry spatialGeometry)
	        throws IOException, FieldNumberOutOfBoundException
	{
		if ((fldNo > 0) && (fldNo <= fldCnt))
		{
			Convert.setSpatialGeometryValue(spatialGeometry,
			        fldOffset[fldNo - 1], data);
			return this;
		}
		else
			throw new FieldNumberOutOfBoundException(null,
	...
	/**
	 * @param fldNo
	 * @return {@link SpatialGeometry}
	 * @throws FieldNumberOutOfBoundException
	 * @throws IOException
	 */
	public SpatialGeometry getSpatialGeometryFld(int fldNo)
	        throws FieldNumberOutOfBoundException, IOException
	{
		SpatialGeometry spatialGeometry;
		if ((fldNo > 0) && (fldNo <= fldCnt))
		{
			spatialGeometry = Convert.getSpatialGeometryShape(
			        fldOffset[fldNo - 1], data);
			...
			switch (types[i - 1].attrType)
				{
					...
					case AttrType.attrString:
						incr = (short) (strSizes[strCount] + 2); // strlen in
						                                         // bytes =
						                                         // strlen +2
						strCount++;
						break;
					case AttrType.attrSpatialGeometry:
						incr = 50;
						break;
			...
			switch (type[i].attrType)
				{
					...
					case AttrType.attrString:
						sval = Convert.getStrValue(fldOffset[i], data,
						        fldOffset[i + 1] - fldOffset[i]);
