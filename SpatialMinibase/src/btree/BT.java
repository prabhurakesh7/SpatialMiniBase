...
public class BT implements GlobalConst
{
	        ...
	        throws KeyNotMatchException
	{
		...
		else if ((key1 instanceof MBRKey) && (key2 instanceof MBRKey))
		{
			double[] coordinates = ((MBRKey) key1).getKey();
			double parent_x1 = coordinates[0];
			double parent_y1 = coordinates[1];
			double parent_x2 = coordinates[2];
			double parent_y2 = coordinates[3];
			
			double[] newLeaf_coordinates = ((MBRKey) key2).getKey();
			double newLeaf_x1 = newLeaf_coordinates[0];
			double newLeaf_y1 = newLeaf_coordinates[1];
			double newLeaf_x2 = newLeaf_coordinates[2];
			double newLeaf_y2 = newLeaf_coordinates[3];
			
			if ((newLeaf_x1 >= parent_x1 && newLeaf_x1 <= parent_x2)
			        && (newLeaf_x2 >= parent_x1 && newLeaf_x2 <= parent_x2)
			        && (newLeaf_y1 >= parent_y1 && newLeaf_y1 <= parent_y2)
			        && (newLeaf_y2 >= parent_y1 && newLeaf_y2 <= parent_y2))
			{
				return 1;
			}
			else
	 ...
	 *
	 * @param key
	 *            specify the key whose length will be calculated.
	 *            Input parameter.
	 ...
	 *                pageType is neither NodeType.LEAF
	 *                nor NodeType.INDEX.
	 */
	 ...
	 *
	 * @param from
	 *            It's a bytes array where KeyDataEntry will come from.
	 *            Input parameter.
	 * @param offset
	 ...
	 *            Input parameter.
	 * @param nodeType
	 *            It specifes NodeType.LEAF or NodeType.INDEX.
	 *            Input parameter.
	 * @param length
	 *            The length of (key, data) in byte array "from".
	 *            Input parameter.
	 * @return return a KeyDataEntry object
	 * @exception KeyNotMatchException
	 *                key is neither StringKey nor IntegerKey
	 * @exception NodeNotMatchException
	 *                nodeType is neither NodeType.LEAF
	 *                nor NodeType.INDEX.
	        ...
	        ConvertException
	{
		...
		try
		{
			...
			else if (keyType == AttrType.attrSpatialGeometry)
			{
				key = new MBRKey(Convert.getDoubleArrValue(offset, from, 32));
			}
			else
				throw new KeyNotMatchException(null, "key types do not match");
	        ...
	        ConvertException
	{
		...
		try
		{
			...
			else if (entry.key instanceof MBRKey)
			{
				Convert.setDoubleArrValue(((MBRKey) entry.key).getKey(), 0,
				        data);
			}
			else
				throw new KeyNotMatchException(null, "key types do not match");
	 ...
	 * @return
	 */
	public static int CoordinatekeyCompare(KeyClass entryKey, KeyClass endKey)
	{
		double[] coordinates = ((MBRKey) endKey).getKey();
		double parent_x1 = coordinates[0];
		double parent_y1 = coordinates[1];
		double parent_x2 = coordinates[2];
		double parent_y2 = coordinates[3];
		
		double[] newLeaf_coordinates = ((MBRKey) entryKey).getKey();
		double newLeaf_x1 = newLeaf_coordinates[0];
		double newLeaf_y1 = newLeaf_coordinates[1];
		double newLeaf_x2 = newLeaf_coordinates[2];
		double newLeaf_y2 = newLeaf_coordinates[3];
	 ...
	 * @return
	 */
	public static boolean IndexRootCoordinatekeyCompare(KeyClass key,
	        KeyClass lo_key)
	{
		double[] coordinates = ((MBRKey) key).getKey();
		double parent_x1 = coordinates[0];
		double parent_y1 = coordinates[1];
		double parent_x2 = coordinates[2];
		double parent_y2 = coordinates[3];
		
		double[] newLeaf_coordinates = ((MBRKey) lo_key).getKey();
		double newLeaf_x1 = newLeaf_coordinates[0];
		double newLeaf_y1 = newLeaf_coordinates[1];
		double newLeaf_x2 = newLeaf_coordinates[2];
		double newLeaf_y2 = newLeaf_coordinates[3];
