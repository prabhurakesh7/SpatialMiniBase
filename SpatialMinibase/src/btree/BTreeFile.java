...
public class BTreeFile extends IndexFile implements GlobalConst
{
	...
	
	{
		...
		{
			if (headerPage.get_keyType() != AttrType.attrSpatialGeometry)
			{
				throw new KeyNotMatchException(null, "");
			}
		}
	        ...
	        PinPageException, UnpinPageException
	{
		...
		while (sortPage.getType() == NodeType.INDEX)
		{
			...
			if (curEntry.key instanceof MBRKey && lo_key instanceof MBRKey)
			{
				if (BT.IndexRootCoordinatekeyCompare(curEntry.key, lo_key))
				{
					while (curEntry != null
					        && lo_key != null
					        && BT.CoordinatekeyCompare(curEntry.key, lo_key) < 0)
					{
