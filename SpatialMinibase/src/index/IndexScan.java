		...
		switch (index.indexType)
			...
			// linear hashing is not yet implemented
				case IndexType.B_Index:
					...
					try
					{
						indScan = (BTFileScan) IndexUtils.BTree_scan(selects,
						        indFile);
					}
					catch (Exception e)
					{
	        ...
	        IOException
	{
		...
		while (nextentry != null)
		{
			if (index_only)
			{
				...
				else if (_types[_fldNum - 1].attrType == AttrType.attrSpatialGeometry)
				{
					attrType[0] = new AttrType(AttrType.attrSpatialGeometry);
					try
					{
						Jtuple.setHdr((short) 1, attrType, s_sizes);
					}
					...
					try
					{
						Jtuple.setDoubleArrayFld(1,
						        ((MBRKey) nextentry.key).getKey());
					}
					catch (Exception e)
					{
						throw new IndexException(e,
